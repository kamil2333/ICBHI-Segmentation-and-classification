...
def get_test_files(dataset_dir):
    ...
    return [f for f in glob.glob(os.path.join(dataset_dir, '*.wav')) if os.path.basename(f).split('_')[0] in test_patients]
...
if not test_files:
    ...
    if st.sidebar.button("Uruchom analizę"):
        with st.spinner('Przetwarzanie nagrania... to może potrwać kilka sekund.'):
            ...
            # Przewidywanie CRNN
            with torch.no_grad():
                probs = torch.sigmoid(crnn_model(
                    torch.tensor(mel_db_seg, dtype=torch.float32).unsqueeze(0).unsqueeze(0).to(device)
                )).squeeze().cpu().numpy()
                
            time_axis = np.arange(len(probs)) * (HOP_LENGTH_SEG / SEG_SR)
            probs_smooth = np.convolve(probs, np.ones(5)/5, mode='same')
            threshold = max(0.07, np.percentile(probs_smooth, 88))
            peaks, properties = find_peaks(probs_smooth, height=threshold, distance=60)
            
            # Filtrowanie szczytów
            min_height = np.mean(probs_smooth) + 0.5 * np.std(probs_smooth)
            peaks = peaks[properties['peak_heights'] > min_height]
            ...
            for s, e, tl in gt_data:
                ax1.add_patch(patches.Rectangle((s, 0), e-s, 1, edgecolor='black', 
                                                facecolor=CLASS_COLORS.get(tl, 'white'), alpha=0.8))
